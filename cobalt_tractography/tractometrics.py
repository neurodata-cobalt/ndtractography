...
def compute_length_vector(segments, debug=False, show_histogram=False):
    ...
    if debug:
        ...
        plt.xlabel('Segment Length (Voxels)')
        plt.ylabel('Probability')
        plt.title('Histogram of Segment Lengths')
#         plt.ylim(0, 1)
        plt.show()
    elif show_histogram:
        tmp1, tmp2, tmp3 = plt.hist(lengths, bins=20)
        plt.xlabel('Segment Length (Voxels)')
        plt.ylabel('Counts')
        plt.title('Histogram of Segment Lengths')
#         plt.ylim(0,1)
...
#     angle_vector, bins, patches = plt.hist(seg_angles, range=(angle_threshold, np.pi/2), density=True, bins=20)
    ...
    if debug:
        ...
        plt.ylabel('Probability')
        plt.title('Histogram of Angles Between Segments')
#         plt.ylim(0, 1)
        plt.show()
    elif show_histogram:
        tmp, tmp2, tmp3 = plt.hist(seg_angles, bins=20)
        plt.xlabel('Angles (Radians)')
        plt.ylabel('Counts')
        plt.title('Histogram of Angles Between Segments')
